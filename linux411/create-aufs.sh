...
rm -rf ../tmp/linux-4.11.7+
mkdir -p ../tmp/linux-4.11.7+
cp -a fs ../tmp/linux-4.11.7+
cp -a include ../tmp/linux-4.11.7+
cp -a Documentation ../tmp/linux-4.11.7+
rm ../tmp/linux-4.11.7+/include/uapi/linux/Kbuild
cd ../tmp
diff -Naur null linux-4.11.7+  | filterdiff | \
sed -e 's|null\(/include/uapi/linux/Kbuild\)|linux-4.11.7+-old\1|;s|^--- null.*|--- /dev/null|;\|linux-4.11.7+/include/uapi/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' | \
bzip2 > aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-4.11.7+/include/uapi/linux/aufs_type.h).patch.bz2
mv *.bz2 $OLDPWD
cd $OLDPWD
mv *patch* ..
cd ..
